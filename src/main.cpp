...
uint256 hashGenesisBlock("0xec46ba6eb6b1f5317a912fd3159c830edb48644bbf8d5c76b625216bfd2007ee");
...
{
	int64 nSubsidy = 0.0000221 * COIN;
	if(nHeight < 10000000) // stops generation of more coins
	{
		nSubsidy = 0.0000221 * COIN;
	}
	if(nHeight < 221)
    {
    ...
    if(nHeight == 2) // On to the Next Case!
    {
       nSubsidy = 0.000221 * COIN;
	}
    if(nHeight == 21)
    {
       nSubsidy = 0.000221 * COIN;
...
static const int64 nTargetTimespan = 600;	// SherlockCoin: 10 minutes
static const int64 nTargetSpacing = 221; // SherlockCoin: 3.6 minute blocks
     ...
     if (BlockLastSolved == NULL || BlockLastSolved->nHeight == 0 || (uint64)BlockLastSolved->nHeight < PastBlocksMin) { return bnProofOfWorkLimit.GetCompact(); }
                ...
                for (unsigned int i = 1; BlockReading && BlockReading->nHeight > 0; i++) {
                        ...
                        if (LatestBlockTime < BlockReading->GetBlockTime()) {
                                if (BlockReading->nHeight > 25600) LatestBlockTime = BlockReading->GetBlockTime();
                        }
...
{
	static const int64	BlocksTargetSpacing			= 3.6 * 60; // 3.6 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlock *pblock)
{
	...
	if (fTestNet) {
		...
		if (pindexLast->nHeight+1 >= 150) { DiffMode = 2; }
	}
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    // fMiner is true when called from the internal sherlockcoin miner
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "SherlockCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xc9f9775b041adb0e6596e22a8fa2485140a14fc60fa4bd000503f3a14a7d0ce0");
    }
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "The Game is on to the CryptoWorld!";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1391796196; //epochtime
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 3366102;

        if (fTestNet)
        {
            block.nTime    = 1391789895;
            block.nNonce   = 2196267;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0xff79255c5379cfc6bd08333e0e3d38874b30edb4304c095959814ddfc354bbf2"));
